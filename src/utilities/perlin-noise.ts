// adapted from https://github.com/haylinmoore/PerlinNoise-Deno/blob/master/mod.ts with some modern Typescript updates,
// addition of a seed for better control over multiple repeatable noise generators with different results,
// and the ability to control the number of dimensions you want noise calculated from.
export class PerlinNoise {
  ...
  constructor(args?: { permutation?: number[]; seed?: number }) {
    if (args?.permutation) {
      this.permutation = args.permutation;
    }

    const seed = args?.seed ?? 0;
    this.p = new Array(512);
    for (let i = 0; i < 256; i++) {
      this.p[256 + i] = this.p[i] = this.permutation[i] ^ (seed & 255);
    }
  }
